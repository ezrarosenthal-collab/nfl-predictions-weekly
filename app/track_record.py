...
def _load_all_predictions(season: int) -> dict[int, dict]:
    ...
    Skips (rather than crashes on) any file that's empty or corrupted --
    same reasoning as scripts/render_html.py's _load_json_safely: these
    files are auto-generated by scheduled workflows that can collide with
    each other, and one bad file shouldn't take down grading for every
    other week that's fine.
    """
    out = {}
    for path in sorted(DATA_DIR.glob(f"predictions_{season}_wk*.json")):
        try:
            ...
            logger.warning("%s isn't valid JSON (corrupted?) -- skipping rather than crashing.", path)
            continue
        out[data["week"]] = data
    return out
