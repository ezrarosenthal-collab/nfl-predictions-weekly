...
def _load_json_safely(path: Path) -> dict | list | None:
    """
    Loads a JSON file, treating "doesn't exist," "empty," and "corrupted/
    unparseable" all the same way: return None rather than crash. This
    matters specifically because these files are auto-generated by
    several independent scheduled workflows -- if one gets manually
    emptied or briefly corrupted during recovery from a git collision (a
    real, recurring situation this project has hit), the render step
    should degrade gracefully to "no data yet" instead of taking down the
    entire site regeneration over one optional file.
    """
    if not path.exists():
        return None
    try:
        ...
        if not content.strip():
            print(f"[render_html] {path} exists but is empty -- treating as no data yet.")
            return None
        return json.loads(content)
    except json.JSONDecodeError:
        print(f"[render_html] {path} exists but isn't valid JSON (corrupted?) -- "
              f"treating as no data yet rather than crashing. It will be regenerated "
              f"the next time its own workflow runs successfully.")
        return None
